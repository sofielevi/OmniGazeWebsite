...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.type || !body.firstName || !body.lastName || !body.email || !body.company) {
      return NextResponse.json(
        { error: "Missing required fields: type, firstName, lastName, email, company" },
        { status: 400 }
      );
    }
    ...
    // Validate type
    if (!["quote", "demo", "community_download"].includes(body.type)) {
