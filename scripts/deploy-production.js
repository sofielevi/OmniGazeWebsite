...
/**
 * OmniGaze Website - Production Deployment Script
 * Deploys to omnigaze.com via FTP
 *
 * Features:
 * - Incremental deployment: only uploads changed files
 * - Uses .ftp-deploy-sync-state-prod.json to track file hashes
...
const STATE_FILE = '.ftp-deploy-sync-state-prod.json';

// FTP Configuration for production
const FTP_CONFIG = {
  ...
  try {
    ...
    try {
      ...
      if (fs.existsSync(HTACCESS_PATH)) {
        await client.ensureDir('/public_html/install');
        await client.uploadFrom(HTACCESS_PATH, '/public_html/install/.htaccess');
        log('.htaccess uploaded to /public_html/install/', 'green');
      }
    } finally {
      client.close();
    }
    ...
    log(`  URL: https://omnigaze.com`, 'yellow');
    console.log('='.repeat(60) + '\n');
    ...
    if (error.message.includes('Login') || error.message.includes('auth')) {
      log('Check FTP credentials in scripts/deploy-production.js', 'yellow');
    }
